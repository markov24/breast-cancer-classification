...
def gradient_descent(iter, X_train, y_train, a, l):
    ...
    for i in range(iter):
        if i%(iter//10) == 0:
            iter_vec.append(i)
            cost_vec.append(compute_cost(X_train, y_train, w_vec, b, l))
            print(f"Iteration: {i}")
...
def predict(X_train, y_train, w_vec, b):
    print(f"Parameters of model are: {w_vec} and {b}")
    m, n = X_train.shape
    correct = 0
    for i in range(m):
        z = np.dot(X_train[i], w_vec) + b
        prediction = sigmoid(z)
        if prediction >= 0.5 and y_train[i] == 1:
            correct += 1
        elif prediction < 0.5 and y_train[i] == 0:
            correct += 1
